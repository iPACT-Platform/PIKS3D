...
integer, parameter :: nKn=5! number of Kn case 36
integer :: iKn
...
double precision, parameter, DIMENSION(1:nKn)::seriesKn=(/10.d0,10.d-1,10.d-2,10.d-3,10.d-4/)
...
! !        If ((abs(i-obstX)<(obstR+1.d-6)).AND.(abs(j-obstY)<(obstR+1.d-6)).AND.(abs(k-obstZ)<(obstR+1.d-6))) then  !rectanguler cross-section
!             image(l) = solid
!         endif
!     Enddo
! Enddo
...
!             !Do i=1,Nx
!                 l=(i+ghostLayer)+(j+ghostLayer-1)*Nxtotal+(k+ghostLayer-1)*Nxytotal
! !                write(20,'(2ES15.3E3, I5)') j, k, image(l)
...
!             !Do i=1,Nx
!                 l=(i+ghostLayer)+(j+ghostLayer-1)*Nxtotal+(k+ghostLayer-1)*Nxytotal
! !                write(20,'(2ES15.3E3, I5)') i, j, image(l)
...
!             !Do i=1,Nx
!                 l=(i+ghostLayer)+(j+ghostLayer-1)*Nxtotal+(k+ghostLayer-1)*Nxytotal
! !                write(20,'(2ES15.3E3, I5)') i, k, image(l)
    ...
    open(20,file='Map_3D.dat',STATUS="REPLACE")
        ...
        Do j=1,Ny
            Do i=1,Nx
                l=(i+ghostLayer)+(j+ghostLayer-1)*Nxtotal+(k+ghostLayer-1)*Nxytotal
        If (image(l)==fluid) then
                    write(20,'(4ES15.3E3)') (i-1)*ds, (j-1)*ds, (k-1)*ds, 0.d0
                else
            write(20,'(4ES15.3E3)') (i-1)*ds, (j-1)*ds, (k-1)*ds, 1.d0
        endif
        Enddo
        Enddo
        Enddo
